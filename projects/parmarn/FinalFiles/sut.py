...
class sut(object):
    def act0(self):
        self.__test.append(('''self.p_count[0]  = 0 ''',self.guard0,self.act0))
        ...
        try:
            ...
            if self.__collectCov:
                self.__cov.stop()
                self.__updateCov()
        self.p_count_used[0]=False
    def guard0(self):
        return (((self.p_count_used[0]) or (self.p_count[0] == None) or (self.__relaxUsedRestriction)))
    ...
    def act11(self):
        self.__test.append(('''self.p_stack[0] = stack.Stack() ''',self.guard11,self.act11))
        if self.__collectCov:
            self.__cov.start()
        try:
            test_before_each(self)
        ...
        try:
            ...
            if self.__collectCov:
                self.__cov.stop()
                self.__updateCov()
        self.p_stack_used[0]=False
    def guard11(self):
        return (((self.p_stack_used[0]) or (self.p_stack[0] == None) or (self.__relaxUsedRestriction)))
    
    def act12(self):
        self.__test.append(('''assert(self.p_count[0]==0) ''',self.guard12,self.act12))
        if self.__collectCov:
            self.__cov.start()
        try:
            test_before_each(self)
        ...
        try:
            ...
            if self.__collectCov:
                ...
                self.__updateCov()
    def guard12(self):
        return (self.p_count[0] != None) and (self.p_stack[0] != None) and (self.p_stack[0].isEmpty())
    
    def act13(self):
        self.__test.append(('''pushOk(self.p_stack[0],self.p_count[0],self.p_int[0]) ''',self.guard13,self.act13))
        if self.__collectCov:
            self.__cov.start()
        try:
            test_before_each(self)
        ...
        self.__warning = None
        try:
            pushOk(self.p_stack[0],self.p_count[0],self.p_int[0])
    ...
    def guard13(self):
        return (self.p_int[0] != None) and (self.p_count[0] != None) and (self.p_stack[0] != None)
    
    def act14(self):
        self.__test.append(('''popOk(self.p_stack[0],self.p_count[0])  ''',self.guard14,self.act14))
        if self.__collectCov:
            self.__cov.start()
        try:
            test_before_each(self)
        ...
        self.__warning = None
        try:
            popOk(self.p_stack[0],self.p_count[0]) 
            ...
            if self.__collectCov:
                ...
                self.__updateCov()
    def guard14(self):
        return (self.p_count[0] != None) and (self.p_count[0] != None) and (self.p_stack[0] != None) and (self.p_count[0]!=0)
    
    def act15(self):
        self.__test.append(('''testOk(self.p_stack[0],self.p_count[0]) ''',self.guard15,self.act15))
        if self.__collectCov:
            self.__cov.start()
        try:
            test_before_each(self)
        ...
        self.__warning = None
        try:
            testOk(self.p_stack[0],self.p_count[0])
            ...
            if self.__collectCov:
                ...
                self.__updateCov()
    def guard15(self):
        return (self.p_count[0] != None) and (self.p_stack[0] != None)
    ...
    def __init__(self):
        try:
            ...
            pass
        self.__modules = []
        self.__modules.append(r"stack.py")
        self.__features = []
        self.__replayBacktrack = False
        self.__cov = coverage.coverage(branch=True, source=["stack.py"])
        ...
        self.p_count = {}
        self.p_count_used = {}
        self.__psize["count"] = 1
        self.__pools.append("self.p_count")
        self.p_count[0] = None
        self.p_count_used[0] = True
        self.p_count[1] = None
        self.p_count_used[1] = True
        self.p_stack = {}
        self.p_stack_used = {}
        self.__psize["stack"] = 1
        self.__pools.append("self.p_stack")
        self.p_stack[0] = None
        self.p_stack_used[0] = True
        self.p_stack[1] = None
        self.p_stack_used[1] = True
        self.p_list = {}
        self.p_list_used = {}
        self.__psize["list"] = 1
    ...
    # END INITIALIZATION CODE
        ...
        self.__actions.append(('''self.p_count[0]  = 0 ''',self.guard0,self.act0))

        self.__names['''self.p_count[0]  = 0 '''] = ('''self.p_count[0]  = 0 ''',self.guard0,self.act0)

        self.__orderings['''self.p_count[0]  = 0 '''] = 1

        self.__okExcepts['''self.p_count[0]  = 0 '''] = ''''''
        ...
        self.__actions.append(('''self.p_stack[0] = stack.Stack() ''',self.guard11,self.act11))

        self.__names['''self.p_stack[0] = stack.Stack() '''] = ('''self.p_stack[0] = stack.Stack() ''',self.guard11,self.act11)

        self.__orderings['''self.p_stack[0] = stack.Stack() '''] = 12

        self.__okExcepts['''self.p_stack[0] = stack.Stack() '''] = ''''''

        self.__actions.append(('''assert(self.p_count[0]==0) ''',self.guard12,self.act12))

        self.__names['''assert(self.p_count[0]==0) '''] = ('''assert(self.p_count[0]==0) ''',self.guard12,self.act12)

        self.__orderings['''assert(self.p_count[0]==0) '''] = 13

        self.__okExcepts['''assert(self.p_count[0]==0) '''] = ''''''

        self.__actions.append(('''pushOk(self.p_stack[0],self.p_count[0],self.p_int[0]) ''',self.guard13,self.act13))

        self.__names['''pushOk(self.p_stack[0],self.p_count[0],self.p_int[0]) '''] = ('''pushOk(self.p_stack[0],self.p_count[0],self.p_int[0]) ''',self.guard13,self.act13)

        self.__orderings['''pushOk(self.p_stack[0],self.p_count[0],self.p_int[0]) '''] = 14

        self.__okExcepts['''pushOk(self.p_stack[0],self.p_count[0],self.p_int[0]) '''] = ''''''

        self.__actions.append(('''popOk(self.p_stack[0],self.p_count[0])  ''',self.guard14,self.act14))

        self.__names['''popOk(self.p_stack[0],self.p_count[0])  '''] = ('''popOk(self.p_stack[0],self.p_count[0])  ''',self.guard14,self.act14)

        self.__orderings['''popOk(self.p_stack[0],self.p_count[0])  '''] = 15

        self.__okExcepts['''popOk(self.p_stack[0],self.p_count[0])  '''] = ''''''

        self.__actions.append(('''testOk(self.p_stack[0],self.p_count[0]) ''',self.guard15,self.act15))

        self.__names['''testOk(self.p_stack[0],self.p_count[0]) '''] = ('''testOk(self.p_stack[0],self.p_count[0]) ''',self.guard15,self.act15)

        self.__orderings['''testOk(self.p_stack[0],self.p_count[0]) '''] = 16

        self.__okExcepts['''testOk(self.p_stack[0],self.p_count[0]) '''] = ''''''
    ...
    def state(self):
        ...
        return [ copy.deepcopy(self.p_int),copy.deepcopy(self.p_int_used),copy.deepcopy(self.p_count),copy.deepcopy(self.p_count_used),copy.deepcopy(self.p_stack),copy.deepcopy(self.p_stack_used),copy.deepcopy(self.p_list),copy.deepcopy(self.p_list_used)]
    def backtrack(self,old):
        ...
        self.p_count = copy.deepcopy(old[2])
        self.p_count_used = copy.deepcopy(old[3])
        self.p_stack = copy.deepcopy(old[4])
        self.p_stack_used = copy.deepcopy(old[5])
        self.p_list = copy.deepcopy(old[6])
        self.p_list_used = copy.deepcopy(old[7])
    def check(self):
        return True
    """
